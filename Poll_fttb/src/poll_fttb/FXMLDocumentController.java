...
//        System.out.println(so.displayoption(ListPoll.getSelectionModel().getSelectedItem().getPoll_id()));
    ...
    @FXML
    private void opt1(ActionEvent event) {
        ServiceOption so = new ServiceOption();
        ServiceVote sv = new ServiceVote();
        sv.INCVote(so.optId(ListPoll.getSelectionModel().getSelectedItem().getPoll_id(),0));
        System.out.println(sv.VoteNbr(so.optId(ListPoll.getSelectionModel().getSelectedItem().getPoll_id(),0)));
    }

    @FXML
    private void opt2(ActionEvent event) {
        ServiceOption so = new ServiceOption();
        ServiceVote sv = new ServiceVote();
        sv.INCVote(so.optId(ListPoll.getSelectionModel().getSelectedItem().getPoll_id(),1));
